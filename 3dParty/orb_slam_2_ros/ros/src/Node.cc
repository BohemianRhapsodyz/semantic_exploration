...
void Node::Init () {
    ...
    pg_server_ = node_handle_.advertiseService("/save_pose_graph", &Node::SavePGSrv, this);
...
void Node::initializeParams(ros::NodeHandle &nh){
    ...
    nh.param<std::string>(name_of_node_+ "/camera_frame_id", camera_frame_id_param_, "camera_link");
    nh.param<std::string>(name_of_node_+ "/target_frame_id", target_frame_id_param_, "base_link");
...
void Node::Update () {
    ...
    if (publish_pointcloud_param_) {
        ...
        getTfTransformMatrix(T_rt, "rgbd_camera", target_frame_id_param_);
...
bool Node::SavePGSrv (orb_slam2_ros::SavePG::Request &req, orb_slam2_ros::SavePG::Response &res) {

    auto vertex_getter=orb_slam_->getVertex();
    std::ofstream outputFile(req.node_file);
    if (!outputFile.is_open()) {
        std::cerr << "Unable to open file: " << req.node_file << std::endl;
        return false;
    }
    int count = 0;
    for (auto it = vertex_getter.cbegin(); it != vertex_getter.cend(); ++it) {
        outputFile << *it << " ";
        count++;

        if (count == 8) {
            outputFile << std::endl; // Start a new line after 8 elements
            count = 0;
        }
    }
    outputFile.close();
    ...
    auto edges_getter=orb_slam_->getEdges();
    std::ofstream outputFile2(req.vert_file);
    if (!outputFile2.is_open()) {
        std::cerr << "Unable to open file: " << req.vert_file << std::endl;
        return false;
    }
    count = 0;
    for (auto it = edges_getter.cbegin(); it != edges_getter.cend(); ++it) {
        outputFile2 << *it << " ";
        count++;

        if (count == 38) {
            outputFile2 << std::endl; // Start a new line after 38 elements
            count = 0;
        }
    }
    outputFile2.close();
