...
from orb_pose_graph.Functions import createMarker, yawBtw2Points
...
class WeightedPoseGraph:
    def __init__(self, nodes: list = None, edges: list = None, criteria: str = 'd_opt'):
        ...
        self.th_plan_points = rospy.get_param('~hallucinated_plan_th', 1)
    ...
    def plotGraph(self, label: str = 'Data', color: str = 'Blue', drawLcSimple: bool = False,
        ...
        if self.graph is not None:
            nodes = self.graph.nodes.data('translation')
            ...
            if drawLcSimple and not drawLcComplex:
                ...
                for (u, v) in edges_LC:
                    poses_LC_x = []
                    poses_LC_y = []
                    poses_LC_x.append(self.graph.nodes[u]['translation'][0])
                    poses_LC_x.append(self.graph.nodes[v]['translation'][0])
                    poses_LC_y.append(self.graph.nodes[u]['translation'][1])
                    poses_LC_y.append(self.graph.nodes[v]['translation'][1])
                    if not label_added:
    ...
    def getGraphAsMarkerArray(self, global_frame: str = "map", color: bool = False) -> MarkerArray:
        ...
        if color:    # generate
            c1 = [1.0, 0.0, 0.0]
            c2 = [1.0, 0.0, 0.0] #line color
            a = 1
            lt = 5
            sc1 = 0.1
            sc2 = 0.04
        else:      #final
            c1 = [1.0, 0.0, 1.0] #node color
            # c2 = [0.0, 0.0, 1.0]
            c2 = [1.0, 0.0, 1.0] #line color
            a = 1.0
            lt = 1
            sc1 = 0.15 #node size in final graph
            sc2 = 0.05  #line size in final graph
    ...
    def hallucinateGraph(self, robot: Robot, orb_map: Map, seen_cells_pct: float, xy_frontier: ndarray, planned_path,
        ...
        # Hallucinate the path to the frontier
        pose_frontier = Pose()
        ...
        # Select certain points along the path
        if n_points > 0:
            if n_points >5:
                plan_nodes = np.ceil(n_points / 2)
                if plan_nodes > 5:
                    plan_nodes = np.ceil(plan_nodes / 2)
                if plan_nodes > 5:
                    plan_nodes = np.ceil(plan_nodes / 2)
                if plan_nodes > 5:
                    plan_nodes = np.ceil(plan_nodes / 2)
                ...
                new_nodes = np.arange(1,n_points)

            # Hallucinate the graph with vertices at location of those points
            ...
            for i in new_nodes:
                ...
                xy_prev_point = [plan[i-1][0], plan[i-1][1]]
                # t_new_point = [plan[i].pose.position.x, plan[i].pose.position.y, plan[i].pose.position.z]
                t_new_point = [plan[i][0], plan[i][1], 0]
                R_new_point = Rotation.from_euler('xyz', [0., 0., yawBtw2Points(xy_prev_point, t_new_point[0:2])],
                                                  degrees=False)
                q_new_point = R_new_point.as_quat()
                planned_pose.orientation.x = q_new_point[0]
                planned_pose.orientation.y = q_new_point[1]
                planned_pose.orientation.z = q_new_point[2]
                planned_pose.orientation.w = q_new_point[3]
                planned_pose.position.x = plan[i][0]
                planned_pose.position.y = plan[i][1]
                planned_pose.position.z = 0
                q_path_point = [planned_pose.orientation.x, planned_pose.orientation.y, planned_pose.orientation.z,
                                planned_pose.orientation.w]

                id_new += 1
                G_frontier.graph.add_node(id_new, translation=t_new_point, orientation=q_path_point)

                # Compute FIMs and create odometry and re localization edges, if any
                edges_with_FIMs = orb_map.hallucinateHessianFromPose(planned_pose, planned_pose, id_last, FIM_last,
                                                                     seen_cells_pct)
            ...
            if return_path:
                ...
                id_markers = 0
                for i in new_nodes:
                    marker = createMarker(mtype="arrow", frame="map", colors=[1.0, 0.0, 0.0], lifetime=4, alpha=0.9,
                                          scale=0.5)
                    xy_prev_point = [plan[i-1][0], plan[i-1][1]]
                    t_new_point = [plan[i][0], plan[i][1], 0]
                    R_new_point = Rotation.from_euler('xyz', [0., 0., yawBtw2Points(xy_prev_point, t_new_point[0:2])],
                    ...
                    marker.pose.position.x = planned_pose.position.x
                    marker.pose.position.y = planned_pose.position.y
                    marker.pose.orientation.x = planned_pose.orientation.x
                    marker.pose.orientation.y = planned_pose.orientation.y
                    marker.pose.orientation.z = planned_pose.orientation.z
                    marker.pose.orientation.w = planned_pose.orientation.w
